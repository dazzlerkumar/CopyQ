...
public:
    ...
    {
        initSingleShotTimer( &m_timer, scriptTimeoutMs, scriptable, SLOT(abort()) );
    }
    ...
    {
        m_saver->displayItem(model, itemData);
        return transformData("displayItem", itemData);
    }
...
private:
    QVariantMap transformData(const QString &fnName, const QVariantMap &itemData)
    {
        ...
        const auto takesTooLong = !m_timer.isActive();
        m_timer.stop();
